...
exports.getVideoList = async (req, res, next) => {
  try {
    ...
    // 한개의 영상의 정보를 조회할 때 official을 체크한다.
    if (fetchList && fetchList.length === 1) {
      isOfficial = await videoQuery.getIsOfficialByVideo({
        videoNo: fetchList[0],
      });
    } else {
    ...
    res.status(200).send({ videoList: result, isOfficial });
  } catch (error) {
    next(error);
  }
};
...
const setupRanking = async (videoNo, gubun) => {
  ...
  setLog("setupRanking", { videoNo, gubun, video, existRanking });
  if (existRanking) {
...
exports.getIndexList = async (req, res, next) => {
  try {
    const { limit, type, user, category_no, videoPage } = req.query;

    // videoPage는 비디오 뷰페이지에서  호출했을 때 true를 반환한다.

    let indexList = [];

    if (String(type).toLowerCase() === "userVideo".toLowerCase()) {
      if (user) {
        console.log("sdfsdf", category_no);
        indexList = await videoQuery.getUserVideoIndex({
          user,
          limit,
          category3No: category_no,
          videoPage,
        });
      } else {
        indexList = [];
      }
    } else if (String(type).toLowerCase() === "my".toLowerCase()) {
      if (req.user) {
        indexList = await videoQuery.getMyVideoIndex({
          me: req.user.user_no,
          limit,
          category3No: category_no,
          videoPage,
      ...
      const parameterOption = { limit, category3No: category_no, videoPage };
      if (String(type).toLowerCase() === "official".toLowerCase()) {
        indexList = await videoQuery.getOfficialVideoIndex({
          category3No: category_no,
        });
      }
      if (String(type).toLowerCase() === "realtimeSurgeLike".toLowerCase()) {
        indexList = await videoQuery.getRealtimeSurgeLikeVideoIndex(
          parameterOption
        );
      }

      if (String(type).toLowerCase() === "realtimeSurgeTalk".toLowerCase()) {
        indexList = await videoQuery.getRealtimeSurgeTalkVideoIndex(
          parameterOption
        );
      }
      if (String(type).toLowerCase() === "hotView".toLowerCase()) {
        indexList = await videoQuery.getHotViewVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "hotTalk".toLowerCase()) {
        indexList = await videoQuery.getHotTalkVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "hotLike".toLowerCase()) {
        indexList = await videoQuery.getHotLikeVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "seasonLike".toLowerCase()) {
        indexList = await videoQuery.getSeasonLikeVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "seasonView".toLowerCase()) {
        indexList = await videoQuery.getSeasonViewVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "new") {
        indexList = await videoQuery.getNewVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "realtiemSurgePush".toLowerCase()) {
        indexList = await videoQuery.getRealtimeSurgePushVideoIndex(
          parameterOption
        );
      }

      if (String(type).toLowerCase() === "hotPush".toLowerCase()) {
        indexList = await videoQuery.getHotPushVideoIndex(parameterOption);
      }

      if (String(type).toLowerCase() === "seasonPush".toLowerCase()) {
        indexList = await videoQuery.getSeasonPushVideoIndex(parameterOption);
      }
    }

    return res.status(200).send({
      message: "success get index list",
      indexList,
    });
  } catch (error) {
    next(error);
...
exports.getCommentsByVideoNo = async (req, res, next) => {
  try {
    const { videoNo } = req.params;
    const comments = await commentQuery.getComment(videoNo);
...
exports.getIsOpenByVideoNo = async (req, res, next) => {
  try {
    const { videoNo } = req.params;
    const isOpen = await videoQuery.getIsOpenByVideoNo({ videoNo });

    return res.status(200).send({ isOpen });
  } catch (error) {
    next(error);
  }
};
 ...
 * 수정할 비디오의 정보 조회
 */
exports.getEditInfoByVideoNo = async (req, res, next) => {
  try {
    const { videoNo } = req.params;
    console.log(req.user);

    const video = await videoQuery.getEditInfoByVideoNo({
      videoNo,
      userNo: req.user && req.user.user_no,
    });

    if (!video) {
      throw createHttpError(400, "비디오가 존재하지 않습니다.");
    }
