 ...
 * 꼭, 백업 후 사용!
 */
PointLogSchema.statics.initData = async function ({ persist }) {
  if (!persist) {
  ...
  const pointLogs = await getAll(`
            SELECT *
            FROM POINT_LOG
          `);
  // console.log(activeLogs);
  await this.deleteMany({});
  await this.create(pointLogs);
};
 ...
 */
PointLogSchema.statics.addDataByDB = async function () {
