...
import { INIT_MAIN_SCROLL, INIT_CATEGORY_SCROLL } from "store/reducers/scroll";
...
const NewHeader = (props) => {
  ...
  const [menuListHeight, setMenuListHeight] = useState("auto");
  const { isLoggedIn } = useSelector(
    (state) => ({ isLoggedIn: state.user.isLoggedIn }),
    shallowEqual
  );
  const { totalPushCount, termsList } = useSelector(
    (state) => ({
      totalPushCount: state.common.totalPushCount,
      termsList: state.common.termsList,
    }),
    shallowEqual
  );
  ...
  const _termsModalOpen = (gubun) => {
    switch (gubun) {
      case "이용 약관":
        _handleMenu();
        return _handleTerms("service");

      case "개인정보 처리 방침":
        _handleMenu();
        return _handleTerms("privacy");

      case "에머겐자 이용 약관":
        _handleMenu();
        return _handleTerms("emergenza");

      case "청소년 보호 정책":
        _handleMenu();
        return _handleTerms("youth");
      default:
        _handleMenu();
        return _handleTerms("service");
    }
  };
  const _handleTerms = (gubun) => {
    if (handleTerms) {
      handleTerms(gubun);
    }
    if (Router.asPath.indexOf("/terms") === -1) {
      Router.push(`/terms?gubun=${gubun}`);
    } else {
      Router.replace(`/terms?gubun=${gubun}`);
    }
  };
  ...
  useEffect(() => {
    setMenuListHeight(`${window.innerHeight}px`);
      ...
      <div className="menuList" onClick={(e) => _handleMenu(e)}>
        <div className="userMenu" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="category_btns">
            ...
            <button
              onClick={() => {
                Router.push("/category/[category_no]", "/category/4").then(() =>
                  _handleMenu()
                );
              }}
            >
            ...
            >
              <span>
                <img
                  src="https://kr.object.ncloudstorage.com/khancomes-bucket001/homepage/logo/logo_pushmysinger_gradation.png"
                  alt="pushmysinger_logo"
                />
              </span>
            </button>
          </div>
          ...
          <div className="terms_group">
            {termsList &&
              termsList.map((terms, i) => (
                <div key={terms.terms_no} className="terms_Link">
                  <span
                    className="terms"
