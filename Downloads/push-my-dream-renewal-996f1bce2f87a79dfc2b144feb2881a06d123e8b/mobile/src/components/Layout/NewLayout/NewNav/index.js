...
const circleItems = [
  ...
  // {
  //   no: 4,
  //   default: wheelButtonDanceBlue,
  //   active: wheelButtonDanceGradation,
  // },
  {
    no: 5,
    default: wheelButtonDreamBlue,
    active: wheelButtonDreamGradation,
    url: ["/category/[category_no]", "/category/4"],
  },
  {
    no: 6,
  ...
  // },
  {
    ...
    url: ["/category/[category_no]", "/category/63"],
  },
];
  ...
  const _handleMenu = (item) => {
    if (item.url.indexOf("http") !== -1) {
      ...
      if (Array.isArray(item.url)) {
        Router.push(...item.url).then(() => _closeMenu());
      } else {
        Router.push(item.url).then(() => _closeMenu());
      }
    }
  ...
  return (
    <div className="container">
      ...
      <span className="upload_btn" onClick={() => Router.push("/upload")}>
        <img src={uploadBtn} alt="upload_btn" width="25px" height="21px" />
      </span>
      <style jsx>{`
        .container {
          position: relative;
