...
import UserPushContainer from "./UserPushContainer";
...
const UserCategoryContainer = ({
  ...
  const isExistCategory = Boolean(
    _.find(
      category3List,
      (category) => category.category_level3_no === Number(category3No)
    )
  );
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    Boolean(category3No && isExistCategory)
      ? {
          value: category3No,
          label: _.find(
            category3List,
            (category) => category.category_level3_no === category3No
          ).category_level3,
        }
      : participations
          .sort(
...
UserCategoryContainer.propTypes = {
...
export default React.memo(UserCategoryContainer);
